...
import psycopg2
blog_data = psycopg2.connect(database="smarten_blog", user="postgres", 
...
app.config["SQLALCHEMY_DATABASE_URI"] = \
    "postgresql://postgres:Blues22#@localhost/smarten_blog"
...
def index():
    """
    The 'homepage' of this blog, essentially. Displays all the articles that 
    are stored in the blog.
    """
    ...
    return render_template("posts.html", article_details=article_details,
...
def login():
    """
    Helps users log in and access the blog if their data has been stored in
    the database.
    """
                ...
                if email_exists:
                    ...
                    if len(password) >= 8  and len(password) <= 16:
                        ...
                        if upper == True and lower == True and digit == True and special == True:
                            ...
                            mycursor.execute(f"INSERT INTO users (username, name, email, password) VALUES \
                                ('{username}', '{name}', '{email}', '{hashed}')")

                            blog_data.commit()

                            flash("Your account was successfully created. Please log in with your new \
                                credentials!", "info")
                            ...
                            requirements = """
                            Invalid Password!! Make sure your password follows all these 
                            requirements:<br/><br/>
                        ...
                        flash("Password length invalid! Password should be between 8-16 characters!",
                         "warning")
...
def posts_individual(post_id):
    """
    Displays the data of each individual post
    """
    ...
    mycursor.execute(f"SELECT name FROM users WHERE id = (SELECT author_no \
        FROM article WHERE article_id = {post_id})")
    ...
    return render_template("posts_individual.html", post=article_details,
        author_name=author_name, tag_names=tag_names, tag_ids=tag_ids,
        len_tags=len_tags, paragraphs=paragraphs)
...
def posts_with_tag(tag_id):
    ...
    mycursor.execute(f"SELECT * FROM article WHERE article_id IN (SELECT article_no \
        FROM post_tags WHERE tag_no = {tag_id})")
    article_details = mycursor.fetchall()
    ...
    return render_template("posts_with_tag.html", posts=article_details,
        author_names=author_names, tag_name=tag_name)
...
def posts_by_author(author_id):
    ...
    return render_template("posts_by_author.html", article_details=article_details,
        author_names=author_names, author_name=author_name)
...
def article_create():
    ...
    """
    Creates an article and stores it in the database
    """
    ...
    if request.method == "POST":
        ...
        mycursor.execute(f"INSERT INTO article (title, summary, content_, \
            image_path, author_no) VALUES ('{title}','{summary}', '{content}',\
            '{image_path}', '{author_id}') RETURNING article_id")
...
def edit_article(post_id):
    ...
    """
    Helps users edit details of a specific article
    """

    mycursor.execute(f"SELECT id FROM users WHERE id = (SELECT author_no FROM \
        article WHERE article_id = {post_id})")
    ...
    if author_id == current_user.id:
        ...
        if request.method == "POST":
            ...
            sql_query = """
            UPDATE article 
            SET title = '{}', summary = '{}', content_ = '{}', image_path = '{}',
                author_no = '{}' 
            WHERE article_id = {}
            ...
            return render_template("article_update.html", title=title,
                                content=content, summary=summary, image_path=image_path, tags=tags,
                                author=author_name)
